  ...
  <AdminLayout>
    <div class="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      <div class="flex flex-wrap gap-3 items-end">
        <div>
          ...
          <input v-model="local.search" @keyup.enter="applyFilters" class="rounded-md border-gray-300 focus:border-gray-900 focus:ring-gray-900" placeholder="Наименование компании" />
        </div>
      ...
      <div class="self-end">
        <Link href="/admin/companies/create" class="inline-flex items-center rounded-md bg-gray-900 text-white px-3 py-2 text-sm hover:bg-black">Создать компанию</Link>
      </div>
    </div>
    ...
    <div class="overflow-x-auto rounded-lg border bg-white">
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr v-for="c in companies.data" :key="c.id" :class="c.deleted_at ? 'opacity-60' : ''" class="border-t">
            ...
            <td class="px-4 py-2 text-right">
              <div class="inline-flex items-center gap-2">
                <Link :href="`/admin/companies/${c.id}/edit`" class="rounded-md border px-2 py-1 text-xs hover:bg-gray-50">Редактировать</Link>
      ...
      <div v-if="companies.links?.length" class="flex justify-between items-center p-3 border-t text-sm">
        <div>Всего: {{ companies.total }}</div>
        <div class="flex flex-wrap gap-1">
          <button
            v-for="l in companies.links"
            :key="l.url + l.label"
            :disabled="!l.url"
            @click="visit(l.url)"
...
const applyFilters = () => {
  router.get('/admin/companies', { search: local.search, show_deleted: local.show_deleted }, { preserveScroll: true })
}
...
const remove = (id) => {
  router.delete(`/admin/companies/${id}`, { preserveScroll: true })
}

const restore = (id) => {
  router.post(`/admin/companies/${id}/restore`, {}, { preserveScroll: true })
}
</script>
