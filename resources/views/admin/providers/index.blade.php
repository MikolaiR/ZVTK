...
@section('content')
    @component('admin.components.page-header', [
        'title' => 'Перевозчики',
        'subtitle' => 'Управление перевозчиками и связью с пользователями',
    ])
        @slot('actions')
            @include('admin.components.button', [
                'href' => route('admin.providers.create'),
                'slot' => 'Добавить перевозчика',
            ])
        @endslot
    @endcomponent
    ...
    @component('admin.components.card')
        <div class="p-4">
            <form method="GET" action="{{ route('admin.providers.index') }}" class="flex flex-wrap items-end gap-3">
                ...
                @if ($filters['search'] ?? $filters['show_deleted'] ?? false)
                    <a href="{{ route('admin.providers.index') }}" class="text-sm text-slate-500 hover:text-slate-700">Сбросить</a>
                @endif
            </form>
        </div>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($providers as $provider)
                        <tr class="{{ $provider['deleted_at'] ? 'bg-slate-50 opacity-75' : '' }}">
                            <td class="px-4 py-3">{{ $provider['id'] }}</td>
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $provider['name'] }}</td>
                            <td class="px-4 py-3">{{ $provider['user']['name'] ?? '—' }}</td>
                            <td class="px-4 py-3">{{ $provider['created_at'] }}</td>
                            <td class="px-4 py-3 text-right">
                                @if ($provider['deleted_at'])
                                    <form method="POST" action="{{ route('admin.providers.restore', $provider['id']) }}" class="inline-block">
                                        @csrf
                                        <button type="submit" class="text-sm font-medium text-emerald-600 hover:text-emerald-700">Восстановить</button>
                                    </form>
                                @else
                                    <a href="{{ route('admin.providers.edit', $provider['id']) }}" class="mr-3 text-sm font-medium text-blue-600 hover:text-blue-700">Изменить</a>
                                    @include('admin.components.delete-form', [
                                        'action' => route('admin.providers.destroy', $provider['id']),
                                        'confirm' => 'Удалить перевозчика?',
                                        'slot' => 'Удалить',
                                    ])
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center text-slate-500">
                                @include('admin.components.empty-state', ['message' => 'Перевозчики не найдены'])
                            </td>
                        </tr>
                    @endforelse
                </tbody>
