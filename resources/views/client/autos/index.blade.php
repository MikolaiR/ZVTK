    ...
    <section class="space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Краткий список с актуальным статусом, локацией и переходом в карточку.
                </p>
            </div>
            @can('create_auto')
        ...
        <form method="GET" action="{{ route('autos.index') }}" class="client-card grid gap-3 p-4 sm:grid-cols-4"
            x-data="{ status: '{{ (string) ($filters['status'] ?? '') }}' }">
            <input type="hidden" name="direction" value="{{ $sort['direction'] ?? 'asc' }}" />
            <div>
                <label for="vin" class="mb-1 block text-sm text-slate-600">VIN</label>
                <input id="vin" name="vin" value="{{ $filters['vin'] ?? '' }}"
            ...
            <div>
                ...
                <select id="status" name="status" x-model="status"
                    x-on:change="if ($event.target.value !== '4') { document.getElementById('parking_id').value = ''; } $el.form.submit()"
                    class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
            ...
            <div x-cloak x-show="status === '4'">
                ...
                <select id="parking_id" name="parking_id" x-on:change="$el.form.submit()"
                    class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
            ...
            <div class="flex items-end gap-2">
                ...
                @if (($filters['status'] ?? null) === '4')
                    <a href="{{ route('autos.export', request()->query()) }}"
                        class="client-btn client-btn-outline">Выгрузить в Excel</a>
                @endif
            </div>
        </form>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-50 text-slate-500">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-medium uppercase tracking-wider">
                                <a href="{{ route('autos.index', array_merge(request()->query(), ['direction' => ($sort['direction'] ?? 'asc') === 'asc' ? 'desc' : 'asc'])) }}"
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse ($autos as $auto)
                            <tr data-parking-highlight="{{ $auto['parking_highlight'] ?? 'none' }}"
                                class="{{ ($auto['parking_highlight'] ?? null) === 'warning' ? 'bg-amber-50 hover:bg-amber-100/80' : (($auto['parking_highlight'] ?? null) === 'danger' ? 'bg-red-100 hover:bg-red-200/80' : 'hover:bg-slate-50/80') }}">
                                <td class="px-4 py-3">
                                    <div class="h-12 w-20 overflow-hidden rounded border border-slate-200">
                                        @if ($auto['preview_url'])
                                            <img src="{{ $auto['preview_url'] }}" alt="{{ $auto['title'] }}"
                                                class="h-full w-full object-cover">
                                        @else
                                            <div
                                                class="flex h-full w-full items-center justify-center bg-slate-100 text-[10px] text-slate-500">
                                                Нет фото</div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <a href="{{ route('autos.show', $auto['id']) }}?back={{ urlencode(url()->full()) }}"
                                        class="font-medium text-slate-800 hover:text-(--client-primary) hover:underline">
                                        {{ $auto['title'] }}
                                    </a>
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="inline-flex items-center rounded px-2 py-0.5 text-xs text-white {{ $statusClasses[(int) $auto['status']] ?? 'bg-slate-800' }}">
                                        {{ $auto['status_label'] }}
                                    </span>
